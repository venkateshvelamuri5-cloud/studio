...
export default function StudentDashboard() {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      setBookingStep(3);
      toast({ title: "Mission Clearance Granted", description: "Your boarding code is now active." });
    } catch {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-body pb-32">
      <header className="px-8 py-6 flex items-center justify-between border-b border-slate-200 bg-white/80 backdrop-blur-xl sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-xl font-black text-slate-900 italic uppercase tracking-tighter leading-none">AAGO</h1><p className="text-[8px] font-black uppercase text-slate-400 tracking-[0.3em] mt-1">{profile?.city} GRID</p></div>
        </div>
        ...
        {activeTab === 'home' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
            <div className="space-y-1">
              ...
              <p className="text-slate-400 font-bold italic text-[10px] uppercase tracking-widest">Vizag Regional Mission Terminal</p>
            </div>
            ...
            {profile?.activeOtp && currentBooking ? (
              <Card className="bg-primary text-white border-none rounded-[3.5rem] p-12 text-center shadow-2xl shadow-primary/30 relative overflow-hidden group">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.5em] mb-12 opacity-70 italic">Verified Boarding Protocol</p>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white/10 p-5 rounded-2xl text-left border border-white/10">
                    <p className="text-[7px] font-black uppercase opacity-60 mb-1">Route Corridor</p>
                  ...
                  {isLoaded ? (
                    <GoogleMap mapContainerStyle={mapContainerStyle} center={currentPosition || { lat: 17.6868, lng: 83.2185 }} zoom={13} options={mapOptions}>
                      ...
                      {activeTrips?.map((trip: any, i: number) => (
                        <Marker key={i} position={{ lat: 17.6868 + (Math.random() - 0.5) * 0.05, lng: 83.2185 + (Math.random() - 0.5) * 0.05 }} icon={{ url: 'https://cdn-icons-png.flaticon.com/512/3448/3448339.png', scaledSize: new window.google.maps.Size(24, 24) }} />
                      ))}
                    </GoogleMap>
                  ) : <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-primary" /></div>}
                  <Button onClick={() => setCurrentPosition(null)} className="absolute bottom-6 right-6 h-12 w-12 rounded-2xl bg-white text-primary shadow-xl p-0"><LocateFixed className="h-6 w-6" /></Button>
                </div>

                <Dialog onOpenChange={(open) => { if (!open) { setBookingStep(1); setSelectedTrip(null); } }}>
                  <DialogTrigger asChild>
                    <div className="p-12 bg-white border border-slate-100 rounded-[3rem] shadow-sm flex items-center justify-between cursor-pointer hover:shadow-xl transition-all border-b-4 border-b-primary group">
                      <div className="space-y-2">
                        <p className="text-[10px] font-black uppercase tracking-widest text-primary">Mission Planning</p>
                  ...
                  <DialogContent className="bg-white border-none rounded-[3.5rem] p-10 max-w-[95vw] sm:max-w-[450px] shadow-2xl h-[85vh] flex flex-col overflow-hidden">
                    <DialogHeader className="mb-6 shrink-0">
                      <DialogTitle className="text-4xl font-black italic uppercase text-primary leading-none">
                        {bookingStep === 1 ? "Route Search" : bookingStep === 2 ? "UPI Protocol" : "Mission Ready"}
                      </DialogTitle>
                      <DialogDescription className="text-[10px] font-bold uppercase tracking-widest mt-3">
                        {bookingStep === 1 ? "Select terminal stations" : bookingStep === 2 ? "Initiate direct payment" : "ID clearance granted"}
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-8 pr-2 custom-scrollbar">
                        ...
                        <>
                          <div className="space-y-4">
                            <div className="relative">
                              ...
                              <select value={pickupStop} onChange={e => setPickupStop(e.target.value)} className="w-full h-18 bg-slate-50 border-none rounded-2xl pl-14 pr-8 font-black italic text-base appearance-none">
                            ...
                            <div className="relative">
                              ...
                              <select value={destinationStop} onChange={e => setDestinationStop(e.target.value)} className="w-full h-18 bg-slate-50 border-none rounded-2xl pl-14 pr-8 font-black italic text-base appearance-none">
                          ...
                          <div className="space-y-4">
                            {filteredTrips.map((trip: any) => (
                              <div key={trip.id} onClick={() => setSelectedTrip(trip)} className={`p-8 rounded-[2.5rem] border-2 flex justify-between items-center transition-all cursor-pointer ${selectedTrip?.id === trip.id ? 'bg-primary border-primary text-white shadow-xl scale-[1.02]' : 'bg-slate-50 border-transparent'}`}>
                                <div>
                      ...
                      {bookingStep === 2 && selectedTrip && (
                        <div className="space-y-10 py-6 text-center">
                          <div className="space-y-4">
                            <div className="h-48 w-48 bg-slate-50 border-4 border-primary/20 rounded-[2.5rem] mx-auto flex items-center justify-center p-6 shadow-inner">
                              <QrCode className="h-full w-full text-slate-900" />
                            </div>
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Scan QR or Use UPI Below</p>
                          </div>
                          <div className="bg-slate-950 p-8 rounded-[2rem] space-y-3">
                            <p className="text-[8px] font-black text-primary uppercase tracking-widest">AAGO PAYMENT ID</p>
                          ...
                          <div className="p-8 bg-primary/5 rounded-[2.5rem] border border-primary/10">
                            <p className="text-[10px] font-black uppercase text-primary tracking-widest mb-1">TOTAL MISSION FARE</p>
                            <h3 className="text-5xl font-black italic text-slate-900">₹{selectedTrip.farePerRider}</h3>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="pt-8 shrink-0">
                      {bookingStep === 1 && (
                        <Button onClick={() => setBookingStep(2)} disabled={!selectedTrip} className="w-full h-18 bg-primary text-white rounded-[1.5rem] font-black uppercase italic text-xl shadow-xl active:scale-95">Initiate Payment</Button>
                      )}
                      {bookingStep === 2 && (
                        <Button onClick={handleConfirmPayment} disabled={isBooking} className="w-full h-18 bg-green-500 text-white rounded-[1.5rem] font-black uppercase italic text-xl shadow-xl active:scale-95">I Have Paid Success</Button>
                      )}
                      {bookingStep === 3 && (
          ...
          <div className="space-y-8 animate-in fade-in text-center">
             ...
             <div className="space-y-4">
                {[ 
                  ...
                  { label: 'Base City', value: profile?.city, icon: MapPin } 
                ].map((item, i) => (
      ...
      <nav className="fixed bottom-0 left-0 right-0 p-8 bg-white/90 backdrop-blur-3xl border-t border-slate-100 z-50 rounded-t-[4rem] shadow-[0_-20px_50px_rgba(0,0,0,0.08)]">
        <div className="flex justify-around items-center max-w-lg mx-auto">
          ...
          <Button variant="ghost" onClick={() => setActiveTab('history')} className={`flex-col h-auto py-3 gap-2 rounded-2xl ${activeTab === 'history' ? 'text-primary' : 'text-slate-400'}`}><History className="h-8 w-8" /><span className="text-[9px] font-black uppercase tracking-widest">History</span></Button>
