...
export default function DriverApp() {
  ...
  const handleToggleStatus = async () => {
    ...
    toast({ title: newStatus === 'available' ? "Operator Signal Active" : "Grid Logout Successful" });
  };
  ...
  const startTrip = async (route: any) => {
    ...
    try {
      ...
      toast({ title: "Mission Initiated", description: `${route.routeName} Corridor` });
  ...
  const verifyPassenger = async () => {
    ...
    if (snap.empty) {
      toast({ variant: "destructive", title: "Invalid Code", description: "Identity sequence refused." });
    } else {
  ...
  const reportNoShow = async () => {
    ...
    toast({ variant: "destructive", title: "Waitlist Notified", description: "No-show logged for current station." });
  };
  ...
  const endTrip = async () => {
    ...
    try {
      ...
      toast({ title: "Mission Accomplished", description: `Fleet Share: ₹${driverPayout.toFixed(0)}` });
    } finally { setIsUpdating(false); }
  };
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-body pb-24 safe-area-inset overflow-x-hidden">
      <header className="px-6 py-6 flex items-center justify-between border-b border-white/5 bg-background/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black italic uppercase tracking-tighter leading-none text-primary text-glow">FLEET OPS</h1>
      ...
      <main className="flex-1 p-5 space-y-6 max-w-lg mx-auto w-full">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-2 gap-4">
              <Card className="bg-white/5 border-white/10 rounded-[2rem] p-6 space-y-2 shadow-xl backdrop-blur-3xl group active:scale-95 transition-all">
                 <p className="text-[9px] font-black uppercase text-muted-foreground tracking-[0.3em]">Fleet Share</p>
              ...
              <Card className="bg-white/5 border-white/10 rounded-[2rem] p-6 space-y-2 shadow-xl backdrop-blur-3xl group active:scale-95 transition-all">
                 <p className="text-[9px] font-black uppercase text-muted-foreground tracking-[0.3em]">Missions</p>
                 <h2 className="text-3xl font-black italic text-foreground leading-none tracking-tighter">{stats.count}</h2>
              </Card>
            </div>

            <h2 className="text-2xl font-black italic uppercase text-foreground tracking-tighter pl-2">Corridor Matrix</h2>
                ...
                <Card key={route.id} className="glass-card rounded-[2rem] shadow-xl overflow-hidden border-white/5 group hover:border-primary/20 transition-all">
                  <CardContent className="p-8 flex justify-between items-center">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] font-black text-primary uppercase tracking-widest">₹{route.baseFare} Payload</p>
                    </div>
                    <Button onClick={() => startTrip(route)} disabled={profile?.status === 'offline' || isUpdating} className="rounded-2xl h-16 px-8 bg-primary text-black font-black uppercase italic text-sm shadow-2xl shadow-primary/20 active:scale-95 transition-all">Launch</Button>
                  </CardContent>
                </Card>
              ))}
            </div>
            ...
            <Card className="glass-card rounded-[3rem] p-10 space-y-8 shadow-2xl relative overflow-hidden border-primary/30">
              <div className="flex justify-between items-start relative z-10">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.4em] mt-2 italic">Active Command</p>
                </div>
                <Badge className="bg-primary/20 text-primary border-none text-[10px] font-black uppercase px-5 py-2.5 rounded-full shadow-lg">{activeTrip.verifiedPassengers?.length || 0} Synced</Badge>
              </div>

              <div className="bg-black/60 p-8 rounded-[2rem] space-y-4 border border-white/10 shadow-inner">
                <Label className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground ml-3 italic">Identity Confirmation</Label>
              ...
              <div className="grid grid-cols-1 gap-4">
                <Button onClick={reportNoShow} variant="outline" className="w-full h-18 bg-white/5 border-white/10 text-muted-foreground rounded-[1.5rem] font-black uppercase italic text-lg active:scale-95 transition-all">
                  <UserX className="mr-3 h-6 w-6" /> Log No-Show
                </Button>
                <Button onClick={endTrip} disabled={isUpdating} className="w-full h-20 bg-primary/10 border-2 border-primary text-primary rounded-[2rem] font-black uppercase italic text-xl shadow-2xl active:scale-95 transition-all">Terminate Mission</Button>
              </div>
            </Card>
          </div>
        ))}
        ...
        {activeTab === 'ledger' && (
          <div className="space-y-6 animate-in fade-in duration-500 pb-12">
             <h3 className="text-4xl font-black italic uppercase text-foreground tracking-tighter pl-2">Fleet Ledger</h3>
                ...
                {[...pastTrips].sort((a,b) => b.endTime.localeCompare(a.endTime)).map((trip: any) => (
                  <Card key={trip.id} className="bg-white/5 border border-white/10 rounded-[1.5rem] p-8 flex justify-between items-center shadow-lg hover:bg-white/10 transition-all cursor-pointer">
                    ...
                    <div className="text-right">
                       ...
                       <p className="text-[8px] font-black text-muted-foreground uppercase mt-1">Settled</p>
                    </div>
                  </Card>
                ))}
             </div>
        ...
        {activeTab === 'fleet' && (
          <div className="space-y-8 animate-in fade-in duration-500">
             <h3 className="text-4xl font-black italic uppercase text-foreground tracking-tighter pl-2">Fleet Health</h3>
             <div className="grid grid-cols-1 gap-6">
                {[
                  { label: "Brakes & Telemetry", value: "98%", icon: Activity, progress: 98 },
                  { label: "Grid Sync Stability", value: "100%", icon: CheckCircle2, progress: 100 },
                  { label: "Cabin Sanitation", value: "95%", icon: ShieldAlert, progress: 95 },
                ].map((item, i) => (
                ...
                <Card className="bg-primary/5 border border-primary/20 rounded-[2.5rem] p-10 flex items-center justify-between shadow-2xl relative overflow-hidden">
                   ...
                   <div className="space-y-2 relative z-10">
                      <p className="text-[10px] font-black uppercase text-primary tracking-[0.4em] italic mb-1">Eco-Mission Score</p>
                      <h3 className="text-5xl font-black italic uppercase text-foreground leading-none tracking-tighter">{stats.carbon} kg <span className="text-sm font-black text-muted-foreground">CO2 Offset</span></h3>
                   </div>
                </Card>
             </div>
          </div>
          ...
          <div className="space-y-12 animate-in fade-in text-center pb-24 pt-10 px-4">
            <div className="flex flex-col items-center gap-10">
              ...
              <div className="space-y-3">
                ...
                <Badge className="bg-primary text-black border-none text-[10px] font-black uppercase px-6 py-2 rounded-full tracking-widest shadow-2xl">{profile?.vehicleType} Commander</Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 text-left">
              {[
                ...
                { label: "Safety Rating", value: "4.95 / 5.0", icon: Star },
                { label: "Hub Location", value: profile?.city || 'Vizag Hub', icon: MapPin }
              ].map((item, i) => (
            ...
            <Button onClick={handleSignOut} className="w-full h-20 bg-destructive/10 text-destructive rounded-[2.5rem] font-black uppercase italic mt-10 border border-destructive/20 active:scale-95 transition-all shadow-2xl text-xl">
              <LogOut className="h-8 w-8 mr-4" /> Exit Terminal
            </Button>
          </div>
        )}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 p-5 bg-background/95 backdrop-blur-3xl border-t border-white/5 z-50 flex justify-around items-center safe-area-inset-bottom shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        ...
        <Button variant="ghost" onClick={() => setActiveTab('profile')} className={`flex-col h-auto py-3 px-5 gap-2 rounded-2xl transition-all ${activeTab === 'profile' ? 'text-primary bg-primary/10 shadow-lg scale-110' : 'text-muted-foreground hover:bg-white/5'}`}>
          ...
          <span className="text-[9px] font-black uppercase tracking-widest">Ops Bio</span>
        </Button>
      </nav>
    </div>
  );
