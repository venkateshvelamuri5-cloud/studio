  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 sm:p-6 font-body safe-area-inset">
      ...
      <div className="mb-6 flex flex-col items-center gap-3 animate-in fade-in duration-1000">
        ...
        <h1 className="text-xl font-black font-headline italic uppercase tracking-tighter text-foreground">OPERATOR ONBOARDING</h1>
      </div>
      ...
      <Card className="w-full max-w-md glass-card border-none rounded-[2.5rem] overflow-hidden shadow-2xl">
        <CardHeader className="pt-8 pb-4 text-center border-b border-white/5 bg-white/5">
          <CardTitle className="text-lg font-black uppercase italic tracking-tighter text-foreground leading-none">Mission Hub</CardTitle>
        ...
        <CardContent className="px-6 py-8 sm:px-10">
          {step === 1 && (
            <div className="space-y-4 animate-in slide-in-from-right-8 duration-500 max-h-[50vh] overflow-y-auto px-1 custom-scrollbar">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} placeholder="DL-0000000" className="h-12 bg-white/5 border-white/10 font-black italic text-base rounded-xl" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" value={experience} onChange={(e) => setExperience(e.target.value)} className="h-12 bg-white/5 border-white/10 font-black italic text-base rounded-xl" />
              </div>
              <Button onClick={() => setStep(2)} disabled={!fullName || !licenseNumber || !aadhaarNumber} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl mt-4">Next Protocol</Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-in slide-in-from-right-8 duration-500 max-h-[50vh] overflow-y-auto px-1 custom-scrollbar">
              ...
              <Button onClick={() => { setStep(3); startCamera(); }} disabled={!vehicleNumber || !seatingCapacity} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl mt-4">Bio Verification</Button>
          ...
          {step === 3 && (
            <div className="space-y-6 text-center animate-in slide-in-from-right-8">
              <div className="relative aspect-square max-w-[200px] mx-auto bg-black/40 rounded-[1.5rem] overflow-hidden border border-white/10 shadow-inner">
                {!photoUrl ? (
              ...
              <Button onClick={() => setStep(4)} disabled={!photoUrl} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl">Link Handset</Button>
              <Button variant="ghost" onClick={() => setStep(2)} className="w-full text-[10px] font-black uppercase italic text-muted-foreground tracking-widest">Go Back</Button>
            </div>
          )}
        ...
        <CardFooter className="bg-white/5 p-8 flex flex-col gap-4 border-t border-white/5">
          <Link href="/driver/login" className="text-[10px] font-black uppercase italic text-primary hover:underline tracking-widest">Exit to Terminal</Link>
