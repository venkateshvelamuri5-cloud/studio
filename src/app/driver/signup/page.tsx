...
export default function DriverSignupPage() {
  ...
  const startCamera = async () => {
    try {
      ...
      toast({ variant: 'destructive', title: 'Camera Blocked', description: 'Enable permissions for ID capture.' });
    }
  };
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStep(5);
      toast({ title: "Signal Sent", description: "Verification code transmitted." });
    } catch (error: any) {
      toast({ variant: "destructive", title: "Auth Error", description: "Could not send code." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Sync Failed", description: "Invalid code sequence." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 font-body safe-area-inset">
      ...
      <Card className="w-full max-w-md glass-card border-none rounded-[3rem] overflow-hidden shadow-2xl">
        <CardHeader className="pt-10 pb-6 text-center">
          <CardTitle className="text-xl font-black uppercase italic tracking-tighter text-foreground leading-none">New Identity</CardTitle>
          <CardDescription className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-2">Initialize Mission Protocol</CardDescription>
        </CardHeader>
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
              <div className="space-y-2">
                ...
                <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Name" className="h-14 bg-white/5 border-white/10 font-black italic text-lg" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">License ID</Label>
                <Input value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} placeholder="DL-XXX" className="h-14 bg-white/5 border-white/10 font-black italic text-lg" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Operational Hub</Label>
                <Select value={city} onValueChange={setCity}>
                  ...
                  <SelectContent className="bg-background border-white/10"><SelectItem value="Vizag">Vizag</SelectItem><SelectItem value="Vizianagaram">VZM</SelectItem></SelectContent>
                </Select>
              </div>
              <Button onClick={() => setStep(2)} disabled={!fullName || !licenseNumber} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl">Vehicle Setup</Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
              <div className="space-y-2">
                ...
                <Input value={vehicleNumber} onChange={(e) => setVehicleNumber(e.target.value)} placeholder="AP-31-XX" className="h-14 bg-white/5 border-white/10 font-black italic text-lg" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Class</Label>
                <Select value={vehicleType} onValueChange={setVehicleType}>
                  <SelectTrigger className="h-14 bg-white/5 border-white/10 text-foreground font-black italic"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-background border-white/10"><SelectItem value="Bus">Bus</SelectItem><SelectItem value="Mini-Bus">Mini-Bus</SelectItem><SelectItem value="Van">Van</SelectItem></SelectContent>
                </Select>
              </div>
              <Button onClick={() => { setStep(3); startCamera(); }} disabled={!vehicleNumber} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl">Capture ID</Button>
            </div>
          )}

          {step === 3 && (
            <div className="space-y-6 text-center animate-in slide-in-from-right-8">
              <div className="relative aspect-square max-w-[240px] mx-auto bg-black/40 rounded-[2rem] overflow-hidden border border-white/10">
                {!photoUrl ? (
              ...
              {!photoUrl ? (
                <Button onClick={capturePhoto} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic">Snap Photo</Button>
              ) : (
                <Button onClick={() => { setPhotoUrl(null); startCamera(); }} variant="ghost" className="text-primary font-black uppercase text-[10px] tracking-widest">Retake</Button>
              )}
              <Button onClick={() => setStep(4)} disabled={!photoUrl} className="w-full bg-accent text-black h-16 rounded-2xl font-black uppercase italic shadow-xl">Handset Verify</Button>
            </div>
          )}
          ...
          {step === 4 && (
            <form onSubmit={handleSendOtp} className="space-y-8 animate-in zoom-in-95">
              <div className="space-y-3">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Signal Phone</Label>
          ...
          {step === 5 && (
            <form onSubmit={handleVerifyOtp} className="space-y-8 animate-in zoom-in-95">
              <div className="space-y-3">
                ...
                <Input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="000000" className="h-20 text-center text-4xl tracking-[0.5em] rounded-2xl bg-white/5 border-white/10 font-black text-primary" maxLength={6} required />
              </div>
        ...
        <CardFooter className="bg-white/5 p-10 flex flex-col gap-6">
          <Link href="/driver/login" className="text-xs font-black uppercase italic text-primary hover:underline">Exit to Login</Link>
