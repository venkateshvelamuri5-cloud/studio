...
export default function DriverSignupPage() {
  const [step, setStep] = useState(1); // 1: Profile & Vehicle, 2: Document Check, 3: Phone verification
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 sm:p-6 font-body safe-area-inset">
      ...
      <div className="mb-6 flex flex-col items-center gap-3 animate-in fade-in duration-700">
        ...
        <h1 className="text-xl font-black italic uppercase tracking-tighter text-foreground text-center">DRIVER REGISTRATION</h1>
      </div>
      ...
      <Card className="w-full max-w-md glass-card border-none rounded-[2.5rem] overflow-hidden shadow-2xl">
        <CardHeader className="pt-8 pb-4 text-center border-b border-white/5 bg-white/5">
          <CardTitle className="text-lg font-black uppercase italic tracking-tighter text-foreground leading-none">Join Hub</CardTitle>
            ...
            <div className="space-y-4 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} placeholder="License" className="h-12 bg-white/5 border-white/10 font-black italic text-base rounded-xl" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Aadhaar No.</Label>
                  <Input value={aadhaarNumber} onChange={(e) => setAadhaarNumber(e.target.value)} placeholder="Aadhaar" className="h-12 bg-white/5 border-white/10 font-black italic text-base rounded-xl" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Plate Number</Label>
                  <Input value={vehicleNumber} onChange={(e) => setVehicleNumber(e.target.value)} placeholder="Plate" className="h-12 bg-white/5 border-white/10 font-black italic text-base rounded-xl" />
                </div>
                <div className="space-y-1.5">
              ...
              <Button onClick={() => { setStep(2); getCameraPermission(); }} disabled={!fullName || !vehicleNumber} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl mt-4 active:scale-95 transition-all">Next: Documents</Button>
            </div>
          )}
            ...
            <div className="space-y-6 text-center animate-in slide-in-from-right-8">
              <div className="relative aspect-[4/3] w-full bg-black rounded-[2rem] overflow-hidden border border-white/10">
                {((captureType === 'profile' && !photoUrl) || (captureType === 'dl' && !dlPhotoUrl) || (captureType === 'aadhaar' && !aadhaarPhotoUrl)) ? (
                  ...
                  <img src={captureType === 'profile' ? photoUrl! : captureType === 'dl' ? dlPhotoUrl! : aadhaarPhotoUrl!} className="h-full w-full object-cover" />
                )}
              ...
              {((captureType === 'profile' && !photoUrl) || (captureType === 'dl' && !dlPhotoUrl) || (captureType === 'aadhaar' && !aadhaarPhotoUrl)) ? (
                <Button onClick={capturePhoto} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-lg active:scale-95 transition-all"><Camera className="mr-2" /> Snap Capture</Button>
              ) : (
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in slide-in-from-right-8">
              <form onSubmit={handleSendOtp} className="space-y-6">
                ...
                {!confirmationResult ? (
                  <Button type="submit" disabled={loading || phoneNumber.length < 10} className="w-full bg-primary text-black h-16 rounded-2xl font-black uppercase italic shadow-xl active:scale-95 transition-all">Verify Phone</Button>
                ) : (
                  <div className="space-y-6">
                     ...
                     <Button onClick={handleVerifyOtp} disabled={loading || otp.length < 6} className="w-full bg-primary text-black h-18 rounded-2xl font-black uppercase italic shadow-xl active:scale-95 transition-all">Complete Signup</Button>
                  </div>
                )}
              </form>
            </div>
        ...
        <CardFooter className="bg-white/5 p-8 flex flex-col gap-4 border-t border-white/5">
          <Link href="/driver/login" className="text-[10px] font-black uppercase italic text-primary hover:underline tracking-widest text-center w-full">Driver Login</Link>
        </CardFooter>
      </Card>
    </div>
  );
